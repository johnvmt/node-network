...
function NodeRouter(config) {
	...
	router._actions = {
		...
		routesupdate: function(routeOperations, connection) {
			...
			// TODO eliminate sending empty arrays
			// TODO cleanup
...
NodeRouter.prototype._routesApplyOperations = function(routeOperations, connection) {
	var router = this;
	var routeOperationsClone = JSON.parse(JSON.stringify(routeOperations)); // TODO remove this

	if(Array.isArray(routeOperationsClone.insert)) {
		// Remove own address
		if(typeof router.address != 'undefined') {
			routeOperationsClone.insert = routeOperationsClone.insert.filter(function (insertOp) {
				return !router._addressManager.addressIsEqual(insertOp.address);
			});
		}

		// Add costs from this router
		routeOperationsClone.insert.forEach(function(insertOp, index) {
			insertOp.cost = insertOp.cost + 1;
		});
	}

	// TODO-remove console.log("ROPS-INS", router.config._id, JSON.stringify(routeOperationsClone));

	var localOperations = router._routeTable.modifyRoutes(routeOperationsClone, connection);
...
NodeRouter.prototype._route = function(packet, callback) {
	...
	// TODO wait for address set before sending?
